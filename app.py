...
def cluster_missing_skills(
    ...
    for s in skills:
        ...
        # Multi-word skill (e.g. "machine learning"): average the
        # vectors of whichever individual tokens exist in the vocab.
        tokens = [t for t in s.split() if t in w2v_model.wv]
        if tokens:
            avg_vec = np.mean([w2v_model.wv[t] for t in tokens], axis=0)
            vectors.append(avg_vec)
            labels.append(s)
        else:
            unvectorizable.append(s)
    ...
    if unvectorizable:
        groups.setdefault("Other / Uncommon Terms", []).extend(unvectorizable)
